...
#     answer = 0
#     def dfs(x):
#         nonlocal computers  #전역의 computers 가져와서 씀
#         for idx, j in enumerate(computers[x]):  #인덱스, 값을 한번에 순회: enumerate함수
#             computers[x][idx] = computers[idx][x] = 0
#             if j and 1 in computers[idx]:
#                 dfs(idx)

#     for i in range(n):  #너무복잡한데 .. 
#         if 1 in computers[i]:
#             dfs(i)
#             answer += 1
#     return answer
...
# computers를 그래프라고 생각하고 순회 
# visited에 각 노드 기록 
# intra=[[1234],[568],[0]] 형식으로 담아놓고 나중에 길이 반환 
...
def solution(n, computers):
    ...
    connection=[]
    def dfs(nodeidx):   
        visited.append(nodeidx)
        connection.append(nodeidx)
        for i in range(n):
            if computers[nodeidx][i] ==1 and i not in visited:
                dfs(i)
                
    for idx in range(n):
        if idx not in visited:
            networks.append(dfs(idx))
            connection=[]
